...
function ExcelSender({ employeeData, warehouses = [], onLoadingChange }) {
  ...
  // ===== NUEVO: Usar el servicio de tu amigo =====
  const {
  ...
  // Calcular taxCodeCountry
  const taxCodeCountry = aplicarIgv ? "01" : "02";
  ...
  useEffect(() => {
    ...
    if (pendingName) {
      ...
      if (savedApplyIgvCost || savedApplyIgvSale) {
        setAplicarIgv(savedApplyIgvCost === 'true' || savedApplyIgvSale === 'true');
      }
    }
  }, []);
  ...
  }, [taxCodeCountry, cargaMode, loading, isLocked, priceLists, selectedPriceLists]);
